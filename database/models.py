...
class BilibiliVideoComment(Base):
    ...
    user_id = Column(String(255), comment='??ID')
    nickname = Column(Text, comment='????')
    sex = Column(Text, comment='??')
    sign = Column(Text, comment='??')
    avatar = Column(Text, comment='??')
    add_ts = Column(BigInteger, comment='?????')
    last_modify_ts = Column(BigInteger, comment='???????')
    comment_id = Column(BigInteger, index=True, comment='??ID')
    video_id = Column(BigInteger, index=True, comment='??ID')
    content = Column(Text, comment='????')
    create_time = Column(BigInteger, comment='?????')
    sub_comment_count = Column(Text, comment='????')
    parent_comment_id = Column(String(255), comment='???ID')
    like_count = Column(Text, default='0', comment='???')

class BilibiliUpInfo(Base):
    __tablename__ = 'bilibili_up_info'
    id = Column(Integer, primary_key=True, comment='??ID')
    user_id = Column(BigInteger, index=True, comment='??ID')
    nickname = Column(Text, comment='????')
    sex = Column(Text, comment='??')
    sign = Column(Text, comment='??')
    avatar = Column(Text, comment='??')
    add_ts = Column(BigInteger, comment='?????')
    last_modify_ts = Column(BigInteger, comment='???????')
    total_fans = Column(Integer, comment='????')
    total_liked = Column(Integer, comment='????')
    user_rank = Column(Integer, comment='????')
    is_official = Column(Integer, comment='??????')

class BilibiliContactInfo(Base):
    __tablename__ = 'bilibili_contact_info'
    id = Column(Integer, primary_key=True, comment='??ID')
    up_id = Column(BigInteger, index=True, comment='UP?ID')
    fan_id = Column(BigInteger, index=True, comment='??ID')
    up_name = Column(Text, comment='UP???')
    fan_name = Column(Text, comment='????')
    up_sign = Column(Text, comment='UP???')
    fan_sign = Column(Text, comment='????')
    up_avatar = Column(Text, comment='UP???')
    fan_avatar = Column(Text, comment='????')
    add_ts = Column(BigInteger, comment='?????')
    last_modify_ts = Column(BigInteger, comment='???????')
...
class BilibiliUpDynamic(Base):
    ...
    id = Column(Integer, primary_key=True, comment='??ID')
    dynamic_id = Column(BigInteger, index=True, comment='??ID')
    user_id = Column(String(255), comment='??ID')
    user_name = Column(Text, comment='????')
    text = Column(Text, comment='????')
    type = Column(Text, comment='????')
    pub_ts = Column(BigInteger, comment='?????')
    total_comments = Column(Integer, comment='????')
    total_forwards = Column(Integer, comment='????')
    total_liked = Column(Integer, comment='????')
    add_ts = Column(BigInteger, comment='?????')
    last_modify_ts = Column(BigInteger, comment='???????')
...
class DouyinAweme(Base):
    ...
    id = Column(Integer, primary_key=True, comment='??ID')
    user_id = Column(String(255), comment='??ID')
    sec_uid = Column(String(255), comment='????ID')
    short_user_id = Column(String(255), comment='???ID')
    user_unique_id = Column(String(255), comment='????ID')
    nickname = Column(Text, comment='????')
    avatar = Column(Text, comment='????')
    user_signature = Column(Text, comment='????')
    ip_location = Column(Text, comment='IP????')
    add_ts = Column(BigInteger, comment='?????')
    last_modify_ts = Column(BigInteger, comment='???????')
    aweme_id = Column(String(64), index=True, comment='??ID')
    aweme_type = Column(Text, comment='????')
    title = Column(Text, comment='????')
    desc = Column(Text, comment='????')
    create_time = Column(BigInteger, index=True, comment='?????')
    liked_count = Column(Text, comment='???')
    comment_count = Column(Text, comment='???')
    share_count = Column(Text, comment='???')
    collected_count = Column(Text, comment='???')
    aweme_url = Column(Text, comment='??URL')
    cover_url = Column(Text, comment='??URL')
    video_download_url = Column(Text, comment='????URL')
    music_download_url = Column(Text, comment='????URL')
    note_download_url = Column(Text, comment='????URL')
    source_keyword = Column(Text, default='', comment='?????')
...
class DouyinAwemeComment(Base):
    ...
    add_ts = Column(BigInteger, comment='?????')
    last_modify_ts = Column(BigInteger, comment='???????')
    comment_id = Column(String(64), index=True, comment='??ID')
    aweme_id = Column(String(64), index=True, comment='??ID')
    content = Column(Text, comment='????')
    create_time = Column(BigInteger, comment='?????')
    sub_comment_count = Column(Text, comment='????')
    parent_comment_id = Column(String(255), comment='???ID')
    like_count = Column(Text, default='0', comment='???')
    pictures = Column(Text, default='', comment='??')
