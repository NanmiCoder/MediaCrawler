...
在原有单实例爬虫链路之外，项目新增了一个独立调度器控制面。调度器不重写各平台抓取逻辑，而是通过 `uv run python main.py` 拉起多个子进程，并为每个实例注入独立的登录态目录、CDP 端口、默认参数和产物目录。
...
flowchart TB
    ...
    manager --> task_a["实例 A 任务子进程"]
    manager --> task_b["实例 B 任务子进程"]
...
- 维护实例配置：平台、登录方式、保存格式、CDP 端口、浏览器 Profile、默认参数。
- 维护任务队列：搜索、详情、创作者和登录任务都绑定到具体实例。
- 管理进程生命周期：实例空闲时启动队列任务，取消任务时终止对应子进程。
...
| `api/scheduler/manager.py` | 任务队列、子进程和产物扫描 |
