...
### 2.3 当前抖音任务与 Content Asset 扩展链路

本节记录当前仓库在通用 MediaCrawler 架构之上的抖音任务化扩展。它描述已落地事实，不代表其他平台都具备相同能力。

```mermaid
flowchart LR
    User["用户 / Web UI"] --> API["FastAPI<br/>宿主端口 18080"]
    API --> TM["TaskManager<br/>独立 task workspace"]
    TM --> Search["search_result"]
    TM --> Comments["comments_raw / comments_clean"]
    TM --> Scripts["script_sources / script_raw / script_clean"]
    Search & Comments & Scripts --> Builder["Content Asset Builder"]
    Builder --> Asset["content_asset.csv / content_asset.jsonl"]
    Asset --> Access["result / preview / GET export"]
    Asset --> Legacy["POST export<br/>旧七字段映射"]
    TM --> WS["/ws/tasks"]
    WS --> User
```
...
- API 宿主访问端口为 `18080`；Docker 容器内部端口为 `8000`。
- Web 开发服务器端口为 `15173`。
- Chrome/CDP 默认端口为 `19222`。
- `web/` 是 Vite/React 源码；构建产物位于 `api/webui/`，由 FastAPI 挂载到 `/ui`。
- merge task 可通过 search/comments/scripts task-id 构建 Content Asset；缺失的真实评论或 ASR 数据通过状态字段表达，不伪装为已完成。

相关结构图：

- [任务/API/Web 类关系](class-diagram.mermaid)
- [任务创建、广播与结果访问时序](sequence-diagram.mermaid)
- [Content Asset 数据字典与验收说明](CONTENT_ASSET.md)
