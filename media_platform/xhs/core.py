...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        start_page = self.start_page  # 您想要开始爬取的页数
        for keyword in self.keyword.split(","):
            utils.logger.info(f"[XiaoHongShuCrawler.search] Current search keyword: {keyword}")
            page = 1
            while (page - start_page + 1) * xhs_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                note_id_list: List[str] = []
                # 如果当前页数小于您想要开始爬取的页数，那么跳过这一页
                if page < start_page:
                    utils.logger.info(f"跳过 page {page}")
                    # notes_res = await self.xhs_client.get_note_by_keyword(keyword=keyword,
                    #                                                       page=page,
                ...
                try:
                    ...
                    utils.logger.error(f"[XiaoHongShuCrawler.search] Search notes {keyword} failed")
                    break
